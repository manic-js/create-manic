...
import { $ } from "bun";
import { red, green, cyan, yellow, dim, brandTitle, PromptSession } from "@manicjs/tui";
  ...
  for (const section of sections) {
    ...
    for (const [name] of entries) {
      ...
      if (!version) {
        ...
        console.log(dim(`  Could not resolve ${name}. Keeping "latest" for now.`));
        continue;
      }
      deps[name] = version;
...
async function ensureBunVersion(options: CliOptions, prompts: PromptSession): Promise<void> {
  ...
  if (options.yes) {
    console.log(
      dim('Cannot prompt for upgrade in --yes mode. Please run "bun upgrade" and try again.'),
    );
    process.exit(1);
  }
  ...
  if (!shouldUpgrade) {
    console.log(dim('Please upgrade Bun manually with "bun upgrade" and rerun create-manic.'));
    process.exit(1);
  }
...
`);
  ...
  const mode = options.yes
    ...
    : (options.mode ?? (await prompts.select("Project mode", ["fullstack", "frontend"], 0)));
