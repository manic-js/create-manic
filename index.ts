...
async function ensureBunVersion(
  ...
  if (!shouldUpgrade) {
    console.log(
      dim(
        'Please upgrade Bun manually with "bun upgrade" and rerun create-manic.'
      )
    );
    process.exit(1);
  }
...
`);
  ...
  const viewTransitions = options.yes
    ...
    : (options.viewTransitions ??
      (await prompts.confirm('Enable View Transitions?', true)));
  const installPackages = options.yes
    ? (options.install ?? false)
    : (options.install ??
      (await prompts.confirm('Install packages now?', true)));
...
`;
  ...
  console.log(
    `${green('Success!')} Created ${cyan(projectName)} at ${cyan(normalizedPath)}`
  );
  console.log(`
${dim('Next steps:')}
